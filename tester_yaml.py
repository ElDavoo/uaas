...
def assert_equal(r, exp_body, exp_sc, req_body=None):
    try:
        if r.status_code != exp_sc:
            return ret_format(ErrorCodes.RC_ERROR, exp_sc, r.status_code, exp_body, get_json(r), r.request.url,
                              r.request.method, req_body)
        if get_json(r) != exp_body:
            return ret_format(ErrorCodes.BODY_ERROR, exp_sc, r.status_code, exp_body, get_json(r), r.request.url,
                              r.request.method, req_body)
        return ret_format(ErrorCodes.OK, exp_sc, r.status_code, exp_body, get_json(r), r.request.url, r.request.method,
                          req_body)
    except ValueError:
        print(r)
...
class TestEndpoints():
    ...
    def update_vars(self, template, obj):
        ...
        if type(template) == str:
            ...
            for k in template.keys():
                try:
                    self.update_vars(template[k], obj[k])
                except:
                    pass
    ...
    def execute_tests(self, t):
        ...
        while i < len(t):
            temp = t[i]
            rv[t[i]['title']] = self.execute_test(t[i])
            new_errorcode = ErrorCodes.get_error(rv[t[i]["title"]]["error"])
            print(f'{t[i]["title"]}\t{new_errorcode}')
            if rv[t[i]["title"]]["error"]:
                print(rv[t[i]["title"]])
                ...
                if errorcode:
                    print("Error fixed, but I need to restart")
                    rv.clear()
                    errorcode = 0
                    t[i] = temp
                    i = 0
                else:
                    errorcode = rv[t[i]["title"]]["error"]
                    i += 1
    ...
    def execute_test(self, t):
        if 'exp_body' in t.keys():
            if t['exp_body'] is not None:
                t['exp_body'] = json_parse(t['exp_body'])
        else:
            t['exp_body'] = None
        if 'body' in t.keys():
            if t['body'] is not None:
                t['body'] = json_parse(t['body'])
        else:
            t['body'] = None
        # de-reference vars
...
if __name__ == "__main__":
    ...
    test = TestEndpoints('http://localhost:5000')
